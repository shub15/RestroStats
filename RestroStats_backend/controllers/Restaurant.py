...
def update_restaurant_profile():
    ...
    if "password" in data: # Hash the new password
        restaurant.password = bcrypt.generate_password_hash(data["password"]).decode("utf-8")
...
def get_peak_hours():
    ...
    peak_hour_transactions = max(hourly_transactions, key=hourly_transactions.get) if hourly_transactions else None
    ...
    return jsonify({
        "peak_hour_sales": f"{peak_hour_sales}:00" if peak_hour_sales is not None else None,
        "peak_hour_transactions": f"{peak_hour_transactions}:00" if peak_hour_transactions is not None else None,
...
def predict():
    ...
    required_fields = ["day_of_week", "hour_of_day", "is_weekend", "is_holiday"]  # Example fields
...
def transactions():
    ...
    # Limit maximum per_page to prevent overwhelming queries
    ...
    # Base query - filter by restaurant_id from JWT
    query = Payment.query.filter_by(restaurant_id=restaurant_id)
    
    # Apply filters if provided
    if item_name:
        query = query.filter(Payment.item_name.ilike(f"%{item_name}%"))
    ...
    paginated_payments = query.order_by(Payment.timestamp.desc()).paginate(page=page, per_page=per_page)
    ...
    for payment in paginated_payments.items:
        payment_dict = {
            ...
            "item_price": float(payment.item_price),
...
def popular_items():
    ...
    # Build query with restaurant_id filter
    query = Payment.query.filter_by(restaurant_id=restaurant_id)
    ...
    # Calculate item popularity by quantity and revenue
    item_stats = df.groupby('item_name').agg({
        'quantity': 'sum',
        'transaction_amount': 'sum'
    }).reset_index()
    
    # Sort by quantity (most ordered)
    top_by_quantity = item_stats.sort_values('quantity', ascending=False).head(5)
    
    # Sort by revenue (highest grossing)
    top_by_revenue = item_stats.sort_values('transaction_amount', ascending=False).head(5)
    
    return jsonify({
        "most_ordered_items": top_by_quantity.to_dict('records'),
        "highest_revenue_items": top_by_revenue.to_dict('records'),
        "top_item": top_by_quantity.iloc[0]['item_name'] if len(top_by_quantity) > 0 else None
    })
...
def insights():
    ...
    if not top_items.empty:
        ...
        insights_list.append(f"Your top selling item is {top_item} with {top_items[0]} units sold.")
    
    # Busiest day of week
    day_sales = df.groupby('day_of_week')['transaction_amount'].sum().sort_values(ascending=False)
    if not day_sales.empty:
        busiest_day = day_sales.index[0]
        insights_list.append(f"Your busiest day is {busiest_day} with ${day_sales[0]:.2f} in sales.")
    
    # Busiest hour
    hour_sales = df.groupby('hour')['transaction_amount'].sum().sort_values(ascending=False)
    if not hour_sales.empty:
        ...
        if hour_12 == 0:
            hour_12 = 12
        insights_list.append(f"Your peak hour is {hour_12} {period} with ${hour_sales[0]:.2f} in sales.")
    ...
    if df['date'].nunique() > 30:
        ...
        if len(monthly_sales) >= 2:
            ...
            growth_rate = ((monthly_sales[last_month] - monthly_sales[previous_month]) / monthly_sales[previous_month]) * 100
            insights_list.append(f"Your monthly sales growth is {growth_rate:.1f}% compared to the previous month.")
