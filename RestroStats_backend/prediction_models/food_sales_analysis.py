...
if df.empty:
    ...
    if os.path.exists(fallback_file):
        try:
            ...
            # Create an empty DataFrame with expected columns to prevent breaking downstream components
            df = pd.DataFrame(columns=['timestamp', 'transaction_type', 'transaction_amount', 'item_name', 'item_type', 'quantity', 'item_price', 'received_by'])
...
def clean_data(df):
    if df.empty:
        print("Warning: Input DataFrame is empty. Returning empty DataFrames with expected structure.")
    ...
    df_clean['transaction_type'] = df_clean['transaction_type'].fillna(df_clean['transaction_type'].mode()[0] if not df_clean['transaction_type'].mode().empty else 'Unknown')
...
def analyze_data(df_clean, df_time_analysis):
    if df_clean.empty or df_time_analysis.empty:
        print("Warning: Empty data provided for analysis. Returning empty analysis results.")
        return {
...
def build_prediction_model(df_time_analysis):
    if df_time_analysis.empty:
        print("Warning: Empty data provided for model building. Skipping model training.")
        return None, [], []
